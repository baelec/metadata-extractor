 ...
 */
class PrintIMDescriptor(directory: PrintIMDirectory) : TagDescriptor<PrintIMDirectory>(directory) {
  override fun getDescription(tagType: Int): String? {
    return when (tagType) {
      PrintIMDirectory.TagPrintImVersion -> super.getDescription(tagType)
      else -> {
